...
class RFL_Model extends CI_Model
{
    ...
    protected $with_user_info       = false;
    protected $created_by_field     = 'created_by';
    protected $updated_by_field     = 'updated_by';
    protected $deleted_by_field     = 'deleted_by';

    public $label_select2_field     = null;
    ...
    protected $_dropdown_dummy_value    = NULL;
    ...
    public function __construct()
    {
        ...
        $this->pagination_cur_tags = (isset($this->pagination_cur_tags)) ? $this->pagination_cur_tags : array('<span>', '</span>');
    ...
    public function _prep_after_read($data, $multi = TRUE)
    {
        ...
        if (isset($this->return_as_dropdown) && $this->return_as_dropdown == 'dropdown') {
            if (!empty($this->_dropdown_dummy_value)) {
                $dropdown[null] = $this->_dropdown_dummy_value;
            }
            foreach ($data as $row) {
    ...
    public function insert($data = NULL)
    {
        ...
        if ($multi === FALSE) {
            if ($this->timestamps !== FALSE) {
                ...
                $data[$this->_updated_at_field] = $this->_the_timestamp(); //! EDIT BY RAFLY | 14 Des 2023 15:53:20 WIB
            }

            if ($this->uuid === TRUE) {
                $data[$this->primary_key] = $this->_the_uuidv4();
            }
        ...
        // else...
            ...
            foreach ($data as $row) {
                if ($this->timestamps !== FALSE) {
                    ...
                    $row[$this->_updated_at_field]  = $this->_the_timestamp(); //! EDIT BY RAFLY | 14 Des 2023 15:53:20 WIB
                }

                if ($this->uuid === TRUE) {
                    $row[$this->primary_key] = $this->_the_uuidv4();
                }
     ...
     * @return mixed
     */
    public function get($where = NULL, $with_trashed = FALSE)
    {
        ...
        if (isset($data) && $data !== FALSE) {
            ...
            if ($this->soft_deletes === TRUE && $with_trashed === FALSE) {
                $this->_where_trashed();
            }
            $this->limit(1);
            $query = $this->_database->get($this->table);
     ...
     * @return mixed
     */
    public function get_all($where = NULL, $with_trashed = FALSE)
    {
    ...
    protected function join_temporary_results($data)
    {
        foreach ($this->_requested as $requested_key => $request) {
            ...
            $with_trashed = array_key_exists('with_trashed', $request['parameters']) ? $request['parameters']['with_trashed'] : FALSE;
            ...
            if (!isset($pivot_table)) {
                ...
                $sub_results = $sub_results->where($foreign_key, $local_key_values)->get_all(null, $with_trashed);
            } else {
                ...
                if (isset($this->{$option}) && !empty($this->{$option})) {
                    foreach ($this->{$option} as $key => $relation) {
                        ...
                        if (!is_array($relation)) {
                            ...
                            //!ADD BY RAFLY : https://github.com/avenirer/CodeIgniter-MY_Model/issues/230
                            if ($model['model_dir']) {
    ...
    public function as_dropdown($field = NULL, $dummy_value = NULL)
    {
        if (!isset($field)) {
        ...
        $this->_dropdown_dummy_value = $dummy_value;
    ...
    public function paginate($rows_per_page, $total_rows = NULL, $page_number = 1, $specific_segment = NULL)
    {
        $this->load->helper('url');
        $segments = !empty($specific_segment) ? $specific_segment : $this->uri->total_segments();
        ...
        if (isset($total_rows)) {
            if ($total_rows != 0) {
                ...
                for ($i = 1; $i <= $number_of_pages; $i++) {
                    ...
                    $uri_string = implode('/', $uri_array);
                    $links .= $i != $page_number ? $this->pagination_delimiters[0] : $this->pagination_cur_tags[0];
                    $links .= (($page_number == $i) ? anchor($uri_string, $i) : anchor($uri_string . '/' . $i, $i));
                    $links .= $i != $page_number ? $this->pagination_delimiters[1] : $this->pagination_cur_tags[1];
                }
    ...
    public function set_pagination_cur_tags($delimiters)
    {
        if (is_array($delimiters) && sizeof($delimiters) == 2) {
            $this->pagination_cur_tags = $delimiters;
        }
        return $this;
    }
    ...
    private function _parse_model_dir($foreign_model)
    {
        ...
        if ($full_model) {
            ...
            if ($data['foreign_model'] !== null && $data['foreign_model'] !== 'null') {
                $data['model_dir'] = preg_replace('/' . preg_quote($data['foreign_model'], '/') . '/', '', implode('/', $full_model), 1);
            } else {
                $data['model_dir'] = implode('/', $full_model);
            }
        }
    ...
    public function add_creator($data)
    {
        if ($this->with_user_info) {
            ...
            $data[$this->created_by_field] = !empty($user_id) ? $user_id : null;
        }
        return $data;
    }
    ...
    public function add_updater($data)
    {
        if ($this->with_user_info) {
            $user_id = $this->session->userdata('userId');
            $data[$this->updated_by_field] = !empty($user_id) ? $user_id : null;
        }
        return $data;
    }
    ...
    public function add_deleter($data)
    {
        if ($this->with_user_info) {
            $user_id = $this->session->userdata('userId');
            $data[$this->deleted_by_field] = !empty($user_id) ? $user_id : null;
        }
        return $data;
    }

    function _the_uuidv4()
    {
        return sprintf(
    ...
    //! CUSTOM BY RAFLY
    function slice($fields = ["*"], $kondisi = [], $page = 1, $perPage = 10, $orderBy = [])
    {
        $totalData                      = $this->where($kondisi)->count_rows();
        $dataSlice                      = $this->fields($fields)
            ->where($kondisi)
            ->order_by($orderBy)
    ...
    {
        if (empty($this->label_select2_field)) {
            if (isset($this->fieldForm()[0]["name"])) {
                $this->label_select2_field = $this->model->fieldForm()[0]["name"];
            } else {
                echo json_encode([
                    "code"      => 404,
        ...
        $label = $this->label_select2_field;
        return $this->fields(["$this->primary_key as value", "$label as label"])->order_by("$label", "ASC")->get_all()   ?: [];
    }
}
